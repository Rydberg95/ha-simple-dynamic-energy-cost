...
- **Monthly bookkeeping**: `export.py` reconstructs a month by replaying recorder history for the energy + price sensors, splitting cost into spot (energy × price) and fixed (energy × fixed_addition). Summaries are persisted to HA storage (`.storage/`) via `export.save_summary`; the `Monthly` sensor saves the just-ended month on its month-boundary reset (delayed 5s so recorder commits the final state). Re-running `export_month` on an old month where recorder history has been purged returns the saved summary instead of overwriting it with zeros (`export.compute_month_summary`). Report files are written to `config/www/` and exposed as `/local/<filename>` URLs on the `LastExportSensor`.
