...
- **Monthly bookkeeping**: `DynamicCostSensor` tracks running `_spot_total`/`_fixed_total`/`_energy_total` counters (persisted via `extra_state_attributes` and restored in `async_added_to_hass`). At the monthly boundary reset (and via a missed-boundary boot check), the Monthly sensor snapshots those live totals to storage via `export.build_live_summary` + `export.save_summary` with `"source": "live"` — this is the authoritative summary and never depends on recorder retention. If no live snapshot exists (e.g. months tracked before v1.5.2), `compute_month_summary` falls back to `export._reconstruct_from_cost_sensor`: the Monthly cost sensor is a running total from 0, so its last recorded state before month end (which always survives `purge_keep_days`) is the month total; the spot/fixed split comes from that state's attributes, or for pre-1.5.2 states from the energy sensor's long-term statistics via `statistics_during_period` with `types={"change"}` (energy = Σ change, fixed = energy × fixed_addition, spot = total − fixed). Last resort is replaying recorder history of the energy + price sensors (`_reconstruct`), which is purge-limited. Summaries carry `complete`/`data_from`/`data_to` coverage fields plus `split_recovered`; `compute_month_summary` never overwrites a `live` summary, never overwrites a `complete` summary with a partial reconstruction, and never replaces a complete summary that has a recovered split with one lacking it. Reports render "okänd" and warning rows when values are missing (PDF/CSV, `LastExportSensor` attributes, ntfy message). Report files are written to `config/www/` and exposed as `/local/<filename>` URLs on the `LastExportSensor`.
