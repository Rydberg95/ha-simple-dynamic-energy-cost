...
**Testing without waiting for the 1st:** call the `simple_dynamic_energy_cost.send_test_notification` entity service on the Last Export sensor (Developer Tools → Actions, or via an automation). It runs the exact same pipeline — exports the previous completed month and pushes the notification — without marking the month as notified. If your self-hosted ntfy server uses a certificate Home Assistant doesn't trust, disable **Verify SSL certificate** in the options (only do this on a trusted LAN).